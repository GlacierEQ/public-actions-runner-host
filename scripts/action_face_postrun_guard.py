#!/usr/bin/env python3
"""Verify runner integrity and lock the result bytes before private publication."""
...
def git(root: Path, *args: str) -> str:
    ...
    if process.returncode != 0:
        fail(f"git {' '.join(args)} failed for {root}")
    return process.stdout.strip()


def verify_checkout(root: Path, expected_sha: str, label: str) -> None:
    if not root.is_dir():
        fail(f"{label} checkout directory does not exist")
    head = git(root, "rev-parse", "HEAD").lower()
    if head != expected_sha:
        fail(f"{label} HEAD does not match the workflow commit")
...
def main() -> int:
    ...
    for field in ("job_id", "pillar", "action", "adapter", "task", "source_repo", "source_ref"):
    ...
    for field in ("pillar", "action", "adapter", "task", "source_repo", "source_ref"):
